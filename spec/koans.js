...
describe('`const` is like `let` plus read-only. ', () => {
  ...
  describe('complex types are NOT fully read-only', () => {
    ...
    it('arrays is not fully read-only', () => {
      const arr = [42, 23];
      arr[0] = 0;
      //This worked because arr is const.
      //i.e it still points to the same array (though its contents have changed, array object remains the same,mutable)
      expect(arr[0]).toBe(0);
    });
    ...
    it('objects are not fully read-only', () => {
      const obj = {x: 1};
      obj.x=2;
      //This worked because obj is const.
      //i.e it still points to the same object (though its attributes have changed, obj object remains the same,mutable)
      expect(obj.x).toBe(2);
    });
...
describe('destructuring can also have default values. ', () => {
  ...
  it('for a missing value', () => {
    //to show that default values do get overridden
    const [a=2,b=2,c=2] = [1,,3];
    expect(b).toEqual(2);
  });
...
describe('spread with strings', () => {
  ...
  it('works anywhere inside an array (must not be last)', function() {
    const letters = ['a', ...'bcd', 'e', ...'fa'];
    expect(letters.length).toEqual(7);
  });
...
describe('class creation', () => {
  ...
  it('multiple methods need no commas (opposed to object notation)', function() {
    class User {
      constructor(){
        this.everWroteTest=false;
      }
      isLazy(){
        return !this.everWroteTest;
      }
      wroteATest(){
        this.everWroteTest = true;
      }
    }
